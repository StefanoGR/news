...
class Api {
  ...
  Future<void> fetchArticles(
      {@required BuildContext context, String category, String query}) async {
    var articlesHolder = Provider.of<ArticlesHolder>(context, listen: false);
    Response response = await dio.get(TOP_HEADLINES,
        options: buildCacheOptions(Duration(seconds: 30), options: Options(extra: createExtras(category, query) )
        ));
